...
在`docker-compose.yml`文件中修改`cas-demo`容器的`environment`字段
...
在`docker-compose.yml`文件中修改容器的`ports`字段
...
将`cfg.json`文件中的`endpoints`字段中`cas server`域名设置为第1步的`CAS_SERVER_NAME`，如第1步中未设置，则设置为`http://localhost:8444`
